...
const PERIOD = ({ setTitle }) => {
    ...
    const url_close = "http://127.0.0.1:8000/api/close_Financial_Period/";
    ...
    useEffect(() => {
        let isMounted = true;
        setTitle("الدورة المالية ")
        // console.log("fetchLeft",user);
        let temp_data = [];
        axios.get(url_periods, { headers: { authorization: "Bearer " + user.accessToken }, })
            .then(response => {
                if (isMounted) {
    ...
    const closeP = (pId) => {
        ...
        axios.post(url_close + pId, user.accessToken, { headers: { authorization: "Bearer " + user.accessToken }, })
            .then((response) => {
    ...
    const update = (e) => {
        ...
        axios.post(url_extend + extendID, formData, { headers: { authorization: "Bearer " + user.accessToken }, })
            .then((response) => {
                console.log("response:", response.data.message);
                console.log(response);
                setMsg(response.data.message);
            ...
            <div className={classes.header}>
                ...
                <div style={{ display: "flex" }}>
                    ...
                    <div className={classes.addIconButton} style={{ marginRight: "0px" }}>
                        <button className={classes.addButton} style={{ borderRadius: "8px", marginTop: "15px", width: "fit-content", borderRadius: "8px" }} onClick={() => setOpen(true)}>إضافة دورة مالية </button>



                    </div>
                    <div style={{ marginRight: "1%" }}>
                        <button className={classes.addButton} style={{ borderRadius: "8px", marginTop: "15px", backgroundColor: "#5d6770", width: "fit-content", borderRadius: "8px" }} onClick={() => reportPeriod()}> تقرير الدورة الحالية </button>
                ...
                <table id="customers" ref={printRef}>
                    ...
                    {data && data.length != 0 && data.map((item) => (
                        <tr>
                            ...
                            <td>{item.endDate}</td>
                            {item.status === "مفتوحة" &&
                                <td style={{ color: "rgb(52, 216, 120)" }}>{item.status}</td>
                            }
                            {item.status === "مغلقة" &&
                                <td style={{ color: "rgb(196, 55, 74)" }}>{item.status}</td>
                            }
                            {!report &&
                                <>
                                    <td><IconButton className={classes.Icons} >
                                    ...
                                    {item.status === "مفتوحة" &&
                                        <td> <IconButton className={classes.Icons} >
                                            <Link to={'/periodOp/' + item.FPId}>
                                                <ArrowBackIosNewIcon className={classes.editIcon} style={{ color: "rgb(162, 52, 177)" }} />
                                            </Link>
                                        </IconButton></td>
                                    }
                                    ...
                                    {item.status === "مفتوحة" &&
                                        <td>   <IconButton className={classes.Icons} >
                                            <CancelOutlinedIcon className={classes.editIcon} style={{ color: "rgb(161, 51, 75)" }} onClick={()=>closeP(item.FPId)} />
                                        </IconButton>
                                        </td>
                                    }
                                </>
                    ...
                    <form action="" >
                        <Grid container style={{ marginTop: "60px" }}>
                            <Grid item xs={11} md={6} lg={6}>
                                <span style={{ color: "#333", fontSize: "20px" }}>تاريخ البدء</span><br />
                                <TextField
                                    id="date"
                            ...
                            <Grid item xs={11} md={6} lg={6}>
                                <span style={{ color: "#333", fontSize: "20px" }}> تاريخ الانتهاء</span><br />
                                <TextField
                                    id="date"
                            ...
                            <Grid item md={6} lg={6} style={{ marginTop: "10px" }}>
                                <div className="label_user_div" style={{ marginTop: "10px" }}> <span style={{ color: "#333", fontSize: "20px" }}>الوصف</span></div>
                    ...
                    <form action="" >
                        ...
                        <>
                            <Grid container style={{ marginTop: "60px" }}>
                                ...
                                <Grid item md={12} lg={12} style={{ marginRight: "10%" }}>
                                    <div className="label_user_div"> <span style={{ color: "#333", fontSize: "20px" }}>تاريخ الانتهاء</span></div>
                            ...
                            <div className={classes.submit}>
                                ...
                                <button className={classes.addButton} style={{ mrginRight: "1200px", marginLeft: "200px", fontSize: "18px", height: "40px" }} onClick={(e) => update(e)}>تعديل</button>
                                <button className={classes.addButton} style={{ backgroundColor: "#6c8997", marginLeft: "20px", height: "40px" }} onClick={() => setOpenEdit(false)}>إلغاء</button>
                            </div>
                        </>
