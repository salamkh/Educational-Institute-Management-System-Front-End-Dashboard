...
const WORKLEAVE = ({ setTitle }) => {
    const classes = useStyles();
    let newDate = new Date();
    let date = newDate.getFullYear() + '-' + (newDate.getMonth() + 1) + '-' + newDate.getDate();
    const url_add = "http://127.0.0.1:8000/api/add_workLeave";
    const url_edit = "http://127.0.0.1:8000/api/edit_workLeave/";
    const url_delete = "http://127.0.0.1:8000/api/delete_workLeave/";
    const url_oneUser = "http://127.0.0.1:8000/api/user_workLeave/";
    const url_range = "http://127.0.0.1:8000/api/range_workLeave";
    ...
    const handleEdit = (item) => {
        setUser(item);
        setName(item.workLeave.userId)
        setDuration(item.workLeave.duration);
        setType(item.workLeave.type);
        setStartDate(item.workLeave.startDate);
        setStartTime(item.workLeave.startTime);
        setOpenEdit(true);
    ...
    const update = (e) => {
        ...
        axios.patch(url_edit + oneUser.workLeave.wId, formData, { headers: { authorization: "Bearer " + user.accessToken }, })
            .then((response) => {
                console.log("response:", response.data.message);
                console.log(response);
                setMsg(response.data.message);
    ...
    const handleDelete = (wId) => {
        axios.delete(url_delete + wId, { headers: { authorization: "Bearer " + user.accessToken }, })
            .then((response) => {
                console.log(response);
                setMsg(response.data.message);
                console.log("res.message", response.data)
    ...
    //     const title = "تقرير الإجازات";
    //     const headers = [["الوقت", "المدة", "التاريخ", "نوع الإجازة", "الاسم"]];

    //     const dataPDF = data.map(item => [item.workLeave.startTime, item.workLeave.duration, item.workLeave.startDate, item.workLeave.type, item.workLeave.userName]);
            ...
            <div className={classes.header}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={11} md={4} lg={4}>
                        <div className={classes.addIconButton} style={{ marginRight: "0" }}>
                            ...
                            <span style={{ color: "#333", fontSize: "20px" }}>  البحث عن الإجازات الخاصة بمستخدم </span>
                ...
                <div style={{ display: "flex" }}>
                    ...
                    <div className={classes.addIconButton} >
                        ...
                        <Tooltip title="إضافة إجازة" area-label="add" >

                            <button onClick={() => setOpen(true)} className="newButton">إضافة إجازة</button>
            ...
            <div className="conTable">
                <table id="customers" ref={printRef}>
                    ...
                    {data && data.length != 0 && data.map((item) => (
                        <tr>
                            <td>{item.workLeave.userName}</td>
                            <td>{item.workLeave.type}</td>
                            <td>{item.workLeave.startDate}</td>
                            <td>{item.workLeave.duration}</td>
                            <td>{item.workLeave.startTime}</td>
                            {!report &&
                                <td>
                                    <div className="social" style={{ marginRight: "20px", bottom: 0, left: 0 }}>

                                        <IconButton className={classes.Icons} >
                                            <DeleteOutlined className={classes.deleteIcon} onClick={() => handleDelete(item.workLeave.wId)} />
                                        </IconButton>
                                        <IconButton className={classes.Icons} >
                ...
                <Container className={classes.modal}>
                    <form action="" >
                        <Grid container style={{ marginTop: "60px" }}>
                            ...
                            <Grid item md={6} lg={6}>
                                <div className="label_user_div"> <span style={{ color: "#333", fontSize: "20px" }}>نوع الإجازة     </span></div>
                                ...
                                >
                                    ...
                                    <MenuItem value="ساعية"> ساعية   </MenuItem>
                            ...
                            <Grid item md={6} lg={6}>
                                <div className="label_user_div" > <span style={{ color: "#333", fontSize: "20px" }}>المدة</span></div>
                            ...
                            {type == "ساعية" &&
                                <Grid item md={12} lg={12}>
                                    <div className="label_user_div" style={{ marginTop: "12px" }}> <span style={{ color: "#333", fontSize: "20px" }}>وقت بداية الإجازة</span></div>
                                ...
                                <Grid container style={{ marginTop: "60px" }}>
                                    <Grid item md={6} lg={6}>
                                        ...
                                        <TextField
                                            ...
                                            label=""
                                            type="text"
                                            value={oneUser.workLeave.userName}
                                    ...
                                    <Grid item md={6} lg={6}>
                                        <div className="label_user_div" style={{ marginTop: "10px" }}> <span style={{ color: "#333", fontSize: "20px" }}>التاريخ</span></div>
                                    ...
                                    <Grid item md={6} lg={6}>
                                        <div className="label_user_div" style={{ marginTop: "10px" }} > <span style={{ color: "#333", fontSize: "20px" }}>المدة</span></div>
