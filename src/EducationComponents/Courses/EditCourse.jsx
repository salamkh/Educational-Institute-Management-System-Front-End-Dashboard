...
export default function EditCourse({ setTitle }) {

    useEffect(() => {
        setTitle("تعديل معلومات دورة")
    }, [])
    ...
    useEffect(() => {
        let isMounted = true;
        fetch(API_KEY + '/showCourse/' + courseId, {
            method: 'GET',
            ...
            .then(response => {
                response.json().then(response => {
                    if (isMounted) {
                        ...
                        setTypeId(response.Course.typeId);
                        setMyClassId(response.Course.classId);
                        setSubjectId(response.Course.subjectId);
                        setTeacherId(response.Course.teacherId);
                        setHeadlines(response.Course.headlines);
                        setAddElements(response.Course.addElements);
                        setEndDate(response.Course.endDate);
                        setStartDate(response.Course.startDate);
                        setStartTime(response.Course.startTime);
                        setSessionNumber(response.Course.sessionNumber);
                        setCost(response.Course.cost);
                        setMaxNStudent(response.Course.maxNStudent);
                        setDuration(response.Course.duration);
                        setCourseDays(response.Course.courseDays);
                        setRoom(response.Course.room);
    ...
    const handleEdit = (e) => {
        ...
        const myCourse = {
            ...
            startDate, endDate, room, duration, courseDays, startTime
        };
        fetch(API_KEY + '/editeCourse/' + courseId, {
            method: 'POST',
            headers: { authorization: "Bearer " + user.accessToken, "Content-Type": "application/json" },
    ...
    return (
        <>
            <div className="test" style={{ marginTop: "70px", marginLeft: "-8%" }}>
                <h4 style={{ color: "rgba(39,116,218,0.5)" }}>تعديل دورة</h4>
                        ...
                        <Grid container>
                            ...
                            <Grid item md={6} lg={6}>
                                <div className={"card"} >
                                    ...
                                    />
                                    <button type="submit" style={{ marginTop: "30px", marginRight: "50%", backgroundColor: "rgb(39,116,218)" }}>
                                        تعديل
                                    </button>
                                    <button style={{ marginTop: "-29px", marginRight: "8%", backgroundColor: "rgba(39,116,218,0.5)" }}>
                                        <a href={'/EducationComponents/Courses/ShowCourse/' + courseId}
                                            style={{
